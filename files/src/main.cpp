...
EmbeddedIocs scan_strings(const std::vector<std::string>& strs) {
    ...
    for (const auto& s : strs) {
        ...
        for (size_t i = 0; i < s.size(); ++i) {
            ...
            if (argus::classify(cand) == argus::IocType::IPv4) found.ips.insert(cand);
            i = j;
        }
        // Interesting tokens
...
std::vector<std::shared_ptr<argus::intel::IntelSource>> build_sources(const Options& o) {
    std::vector<std::shared_ptr<argus::intel::IntelSource>> v;
    if (!o.vt_key.empty())    v.push_back(std::make_shared<argus::intel::VirusTotal>(o.vt_key));
    if (!o.abuse_key.empty()) v.push_back(std::make_shared<argus::intel::AbuseIPDB>(o.abuse_key));
    return v;
}

void print_source_result(const argus::intel::SourceResult& r) {
    using namespace argus::ui;
    if (!r.ok) {
...
void usage() {
    std::cout <<
    "ARGUS - IOC enrichment & PE static analysis\n\n"
    "usage: argus <indicator-or-file> [options]\n\n"
    ...
    "  argus 185.220.101.5 --abuse-key $env:ABUSEIPDB_API_KEY\n"
    "  argus https://sketchy.example/payload.bin --json\n";
}
...
int main(int argc, char** argv) {
    ...
    argus::ui::init();
    if (opts.no_color) argus::ui::color_enabled() = false;

    if (opts.help || opts.target.empty()) {
        if (!opts.json) argus::ui::banner();
        usage();
